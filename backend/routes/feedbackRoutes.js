...
router.post("/fillFeedback", async (req, res) => {
  try {
    ...
    const latestFeedback = await Feedback.findOne().sort({ firNumber: -1 });
    const firNumber = latestFeedback ? latestFeedback.firNumber + 1 : 1;
    ...
    res.status(201).json({ message: "Feedback submitted successfully!", firNumber });
  } catch (error) {
    console.error(error);
...
router.post('/fillPostFirFeedback', async (req, res) => {
  try {
    ...
    // Send FIR number back to frontend
    res.status(201).json({ message: 'Post-FIR Feedback submitted successfully!', firNumber });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Internal Server Error' });
  }
